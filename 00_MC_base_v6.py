...
Took end-user-feedback onboard - fixed discovered errors/bugs
 ...
 - Fixed when all cards are deleted, system no longer crashes through find_monster() function/component
"""
    ...
    while confirm != "Yes - Quit":
        ...
        if proceed == "Exit":  # if user wants to exit, ask them to confirm exit
            confirm = eg.buttonbox("Please confirm that you want to exit the program.",
                                   "Confirm Exit", choices=("Yes - Quit", "No - Cancel"))
    ...
    while user_help != "Cancel":  # a while loop checking that the user hasn't clicked 'Cancel'
        if user_help == "'Find'":  # If user clicks 'Find', display information about 'Find' Button
            ...
            eg.msgbox("This is accessed from a button on the main menu called 'Exit'. Clicking it will prompt you to "
                      "confirm that you want to exit the program.\n\n*Important Notice* - You will be prompted on "
                      "whether or not you want to save your progress after quitting. Only one set of data can be saved "
                      "at a time. Upon starting the program up again, you will be prompted for if you want to import "
                      "the saved data. Clicking yes will load saved data, clicking no will load the default values "
                      "that are saved to the dictionary.", "'Exit' Help")
        user_help = eg.buttonbox("What do you need help with?", "HELP MENU",
